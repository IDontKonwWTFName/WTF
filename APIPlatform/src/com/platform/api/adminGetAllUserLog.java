...
@WebServlet("/adminGetAllUserLog")
public class adminGetAllUserLog extends HttpServlet{
	@Override
	...
	try {
		SQLQuery sqlQuery =session.createSQLQuery("select top 500 * from dbo.[User_log] order by time asc").addEntity(User_log.class);
		List<User_log> uls =sqlQuery.list();
		JSONObject jsonObject=null;
		JSONArray jsonArray=new JSONArray().fromObject(uls);
